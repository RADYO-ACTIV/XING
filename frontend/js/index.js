...
function cacheElements() {
  ...
  elements.intro = document.getElementById('countdown');
...
async function showSummaryDialog() {
  ...
  try {
    ...
    elements.loading.textContent = 'Aww Snap, Couldn\'t load questions at this point. Try Again Later'
...
function startGame() {
  ...
  LAST_QUESTION_INDEX = (parseInt(gameState.config.questions)-1)
  // Show quiz interface
...
function setupOptionClickHandlers() {
  ...
  optionClickHandler = (event) => {
    ...
    if (questionIndex !== gameState.currentIndex) return;
...
function handleNextQuestion() {
  ...
  if (gameState.currentIndex <= LAST_QUESTION_INDEX) {
    displayCurrentQuestion();
  }else {
    endQuiz();
  }
...
function analysis(score, total){
  if (score <= (total*0.4)){
    return 'Your Score is Very Low'
  }else if(score <= (total*0.5)-1){
